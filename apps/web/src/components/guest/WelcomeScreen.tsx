  ...
  return (
    <main className="relative flex min-h-screen flex-col overflow-hidden bg-ink-900 px-6 text-white">
      ...
      <div className="relative z-10 flex flex-1 flex-col justify-center py-14">
        <span className="animate-rise-in text-sm font-bold uppercase tracking-[0.2em] text-brand-300">
          {restaurantName}
        </span>

        <h1 className="mt-3 animate-rise-in text-[34px] font-extrabold leading-[1.1] tracking-[-0.02em]" style={{ animationDelay: '60ms' }}>
        ...
        >
          <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-brand-500 text-xl shadow-pill" aria-hidden>
            🪑
          </span>
          <div className="min-w-0">
            <p className="text-xs font-medium uppercase tracking-wider text-white/50">You&apos;re seated at</p>
            <p className="truncate text-lg font-extrabold">{tableLabel}</p>
            <p className="text-xs text-white/60">{zone}</p>
          </div>
        </div>

        <ol className="mt-7 animate-rise-in space-y-3" style={{ animationDelay: '240ms' }}>
          {[
            ['🧾', 'Browse and add', 'Order as many rounds as you like.'],
            ['🔥', 'We start cooking', 'It reaches the kitchen the moment you send it.'],
            ['💳', 'Pay when you\u2019re ready', 'Ask for the bill right here — one bill for the table.'],
          ].map(([icon, title, body], i) => (
            <li key={title} className="flex items-start gap-3">
              <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-white/10 text-sm ring-1 ring-white/10" aria-hidden>
                {icon}
              </span>
              <div className="min-w-0 flex-1 pt-0.5">
                <p className="text-sm font-bold leading-tight">
                  <span className="mr-1.5 text-white/40">{i + 1}.</span>{title}
                </p>
