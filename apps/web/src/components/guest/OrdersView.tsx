        ...
        return (
          <div key={step.key} className="flex flex-1 flex-col items-center">
            <div className="flex w-full items-center">
              ...
              <span
                className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[11px] transition ${
            ...
            <span className={`mt-1.5 text-[10px] font-semibold ${active ? 'text-brand-600' : done ? 'text-ink-600' : 'text-ink-400'}`}>
              {step.label}
            </span>
          </div>
        );
  ...
  if (orders.length === 0) {
    return (
      <div className="px-4 py-16 text-center">
        ...
        <p className="mt-4 text-lg font-bold text-ink-800">Nothing ordered yet</p>
    ...
    <div className="space-y-4 px-4 pb-28 pt-1">
      {live.length > 0 && (
        <div className="flex items-center gap-2.5 rounded-2xl bg-brand-50 px-4 py-3 ring-1 ring-brand-200">
          ...
          <p className="text-sm font-semibold text-brand-800">
            {live.length} {live.length === 1 ? 'round is' : 'rounds are'} with the kitchen
          </p>
        </div>
      )}
      ...
      {orders.map((order, i) => (
        ...
        >
          <header className="flex items-center justify-between px-4 pb-3 pt-4">
            <div>
              <p className="text-sm font-extrabold text-ink-900">Round #{order.order_number}</p>
              <p className="text-xs text-ink-400">{time(order.created_at)}</p>
            </div>
            <span className="text-sm font-extrabold text-ink-900">{money(order.subtotal, symbol)}</span>
          </header>
      ...
      <section className="card overflow-hidden" aria-label="Running bill">
        <div className="bg-ink-900 px-4 py-3">
          <h2 className="text-sm font-extrabold text-white">Your bill so far</h2>
        </div>
        <dl className="space-y-2 p-4 text-sm">
          ...
          <div className="flex items-baseline justify-between border-t border-dashed border-ink-200 pt-3">
            ...
            <dd className="text-2xl font-extrabold text-ink-900">{money(totals.total, symbol)}</dd>
          </div>
        </dl>
      </section>
      ...
      {settled && bill && (
        <div className="rounded-2xl bg-emerald-50 px-4 py-4 text-center ring-1 ring-emerald-200">
          ...
          <p className="mt-1.5 text-sm font-bold text-emerald-900">Paid in full — thank you!</p>
      ...
      {!settled && (
        ...
        >
          <div className="mx-auto flex max-w-2xl gap-2">
            ...
            >
              {billRequested ? 'Bill requested ✓' : 'Ask for the bill'}
            </button>
          </div>
        </div>
      )}
